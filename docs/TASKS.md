...
- [x] T1533 将通知设置规则、通知模板和通知日志迁移到 `TableToolbar` 保存视图范式，并接入后端排序白名单
...
- [x] T1551 统一后台顶栏刷新、页面刷新注册、非左侧入口路由预加载和实时后台刷新失效协议，避免手动刷新或按钮跳转强制重挂页面
- [x] T1552 固定工作区已打开页签的原始位置，并强化页签关闭操作按钮的视觉层级
- [x] T1553 移除所有列表页通用工具栏底部的默认筛选、显示列和密度状态提示条
- [x] T1554 全站移除列表工具栏的紧凑、标准、宽松密度切换控件
...
- [x] T0205 前端接入客户管理、来源平台设置、发货模板、附件中心真实页面
